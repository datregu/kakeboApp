...
const ModalWindowUpdateIncome = ({ open, handleClose, title, income, userId, setIsIncomeUpdated }) => {
    const user_id = 1;
            ...
            <Fade in={open}>
                <Box sx={{
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <TextField name="incomeAmount" label="Cantidad" value={incomeData.incomeAmount} onChange={handleChange}/>
                        <input
                            type="date"
                            value={incomeData.incomeDate instanceof Date ? incomeData.incomeDate.toISOString().substr(0, 10) : ''}
                            onChange={(event) => {
                                setIncomeData({
                                    ...incomeData,
                        ...
                        />
                        <TextField name="incomeDescription" label="Descripción" value={incomeData.incomeDescription} onChange={handleChange}/>
                    </Box>
