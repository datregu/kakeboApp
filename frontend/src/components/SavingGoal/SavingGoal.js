...
const SavingGoal = ({ monthlyRecord, onSavingsChange }) => {
    ...
    return (
        <Box style={{ display: 'flex', flexDirection: 'column' }} className="setSavingsBox">
            {showInput ? (
                ...
                <>
                    {desiredSavings === 0 ? (
                        ...
                        <>
                            <div>
                                Presupuesto mensual inicial: {monthlyRecord ? monthlyRecord.total_income - monthlyRecord.fixed_expenses - desiredSavings : 0} €
            ...
            <div>Presupuesto mensual final: {monthlyRecord ? monthlyRecord.total_income - monthlyRecord.fixed_expenses : 0} €</div>
        </Box>
    );
};
