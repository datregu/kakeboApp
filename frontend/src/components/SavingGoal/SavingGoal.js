        ...
        fetch(url, { method: 'POST' })
            ...
            .then(text => {
                try {
                    ...
                    alert('Se ha establecido un nuevo ahorro objetivo, pero la respuesta del servidor no fue como se esperaba.');
                }
            ...
            {isInputDisabled && (
                <>
                    <Typography variant="h5" sx={{ color: '#388e3c' }}>
                        {monthlyRecord ? (monthlyRecord.total_income - monthlyRecord.fixed_expenses).toFixed(2) : 0} €
                    </Typography>
                    ...
                    <Button variant="outlined" onClick={handleButtonClick} sx={{ marginTop: 2 }}>Cambiar ahorro objetivo</Button>
                </>
            )}
            <Typography variant="h5" sx={{ color: 'blue', marginTop: 2 }}>
                {monthlyRecord ? (monthlyRecord.total_income - monthlyRecord.fixed_expenses - desiredSavings).toFixed(2) : 0} €
            </Typography>
            <Typography variant="body2" color="textSecondary">
